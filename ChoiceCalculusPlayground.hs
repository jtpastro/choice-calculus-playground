...
type DimId = String
type DimDef = (DimId, [TagId])
type TagId = String

indexOf::DimDef -> TagId -> Maybe Int
indexOf(d,ts) t = elemIndex t ts
...
data V a = Obj a -- Não precisamos retornar o objeto em uma estrutura de árvore somente retornamos a representação do objeto na Object Language
    | Dim DimDef (V a) -- Definindo uma nova dimenção 'Dim' com nome 'DimId', as tags 'TagId' e uma expressão variacional
...
-- A sacada: a expressão retornada por 'atomic' é justamente uma única expressão Chc
atomic :: DimDef -> [V a] -> V a
...
choiceElimination::V a -> DimId -> TagId -> V a
...
choiceElimination (Chc d' vs) d t
    | d' == d = choiceElimination (vs !! indexOf t) d t 
    | otherwise = Chc d $ map (\v' -> choiceElimination v' d t) vs

type Choice = (DimId, TagId)
